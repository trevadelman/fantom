...
class Month(Obj):
    ...
    def toStr(self): return self._name
    def __str__(self): return self._name

    @staticmethod
    def jan(): return Month._get(0)
    @staticmethod
    def feb(): return Month._get(1)
    @staticmethod
    def mar(): return Month._get(2)
    @staticmethod
    def apr(): return Month._get(3)
    @staticmethod
    def may(): return Month._get(4)
    @staticmethod
    def jun(): return Month._get(5)
    @staticmethod
    def jul(): return Month._get(6)
    @staticmethod
    def aug(): return Month._get(7)
    @staticmethod
    def sep(): return Month._get(8)
    @staticmethod
    def oct(): return Month._get(9)
    @staticmethod
    def nov(): return Month._get(10)
    @staticmethod
    def dec(): return Month._get(11)
    ...
    def _get(ordinal):
        ...
        return Month._vals[ordinal]

    @staticmethod
    def vals():
        if Month._vals is None:
            Month._get(0)  # Initialize
        from .List import List
        return List.toImmutable(List.fromList(Month._vals[:]))
    ...
    def fromStr(s, checked=True):
        ...
        if s_lower in names:
            return Month._get(names.index(s_lower))
        if checked:
            from .Err import ParseErr
            raise ParseErr.make(f"Unknown month: {s}")
        return None

    def numDays(self, year):
        """Get number of days in this month for given year"""
    ...
    def increment(self):
        """Return next month (wraps from Dec to Jan)"""
        return Month._get((self._ordinal + 1) % 12)

    def decrement(self):
        """Return previous month (wraps from Jan to Dec)"""
        return Month._get((self._ordinal - 1) % 12)

    def __add__(self, val):
        """Month + Int -> Month"""
        return Month._get((self._ordinal + val) % 12)
    ...
    def toLocale(self, pattern=None, locale=None):
        """Format month for locale"""
        if pattern is None:
            pattern = "MMM"
        if pattern == "":
            from .Err import ArgErr
            raise ArgErr.make("Empty pattern")
        if len(pattern) > 4 or (len(pattern) > 0 and pattern[0] != 'M'):
            from .Err import ArgErr
            raise ArgErr.make(f"Invalid Month pattern: {pattern}")
    ...
    def localeAbbr(self):
        """Get abbreviated locale name"""
        return self.toLocale("MMM")

    def localeFull(self):
        """Get full locale name"""
        return self.toLocale("MMMM")
...
class DateTime(Obj):
    ...
    _cachedUtc = None   # Cached DateTime.nowUtc() value
    ...
    @staticmethod
    def _daysSinceEpoch(year, month_ord, day):
        ...
        if year >= 2000:
            ...
            for y in range(2000, year):
                total_days += 366 if DateTime._isLeapYear(y) else 365
        else:
    ...
    @staticmethod
    def _computeTicks(year, month_ord, day, hour, min_, sec, ns, tz_offset_secs=0):
        ...
        days = DateTime._daysSinceEpoch(year, month_ord, day)
    ...
    def __init__(self, year=2000, month=None, day=1, hour=0, min_=0, sec=0, ns=0, tz=None):
        ...
        try:
            ...
            rule = tz._getRule(year) if hasattr(tz, '_getRule') else None
            if rule is not None:
                ...
                tz_offset_secs = offset_dur.toSec() if hasattr(offset_dur, 'toSec') else offset_dur
        ...
        self._ticks = DateTime._computeTicks(year, month_ord, day, hour, min_, sec, ns, tz_offset_secs)
    ...
    def now(tolerance=_USE_DEFAULT_TOLERANCE):
        ...
        # Get current ticks
        now_ticks = DateTime._nowTicksRaw()
        ...
        if DateTime._cached is None:
            DateTime._cached = DateTime.makeTicks(0, TimeZone.cur())
        ...
        DateTime._cached = DateTime.makeTicks(now_ticks, TimeZone.cur())
    ...
    @staticmethod
    def nowUtc(tolerance=_USE_DEFAULT_TOLERANCE_UTC):
        ...
        if DateTime._cachedUtc is None:
            DateTime._cachedUtc = DateTime.makeTicks(0, TimeZone.utc())
        ...
        # Update cache and return new value
        DateTime._cachedUtc = DateTime.makeTicks(now_ticks, TimeZone.utc())
    ...
    @staticmethod
    def _makeWithOffset(year, month, day, hour, min_, sec, ns, offset_secs, tz):
        ...
        ticks = DateTime._computeTicks(year, month_ord, day, hour, min_, sec, ns, offset_secs)
        ...
        try:
            ...
            std_offset = std_offset_dur.toSec() if hasattr(std_offset_dur, 'toSec') else std_offset_dur
            dst_offset_dur = tz.dstOffset(year)
            dst_offset = dst_offset_dur.toSec() if hasattr(dst_offset_dur, 'toSec') else 0
    ...
    @staticmethod
    def fromStr(s, checked=True, tz=None):
        ...
        try:
            ...
            if day < 1 or day > month_obj.numDays(year):
            ...
            if offset_secs is not None:
                return DateTime._makeWithOffset(year, month, day, hour, minute, sec, ns, offset_secs, tz)
    ...
    def nanoSec(self): return self._ns
    ...
    def dst(self):
        ...
        """
        try:
            rule = self._tz._getRule(self._year) if hasattr(self._tz, '_getRule') else None
    ...
    def tzAbbr(self):
        ...
        if self.dst():
            return self._tz.dstAbbr(self._year)
        return self._tz.stdAbbr(self._year)
    ...
    def toStr(self):
        ...
        if self._tz == TimeZone.rel():
            return self.toLocale("YYYY-MM-DD'T'hh:mm:ss.FFFFFFFFF")
        return self.toLocale("YYYY-MM-DD'T'hh:mm:ss.FFFFFFFFFz zzzz")
    ...
    def plus(self, duration):
        ...
        if d == 0:
            return self
        return DateTime.makeTicks(self._ticks + d, self._tz)
    ...
    def __sub__(self, other):
        ...
        if isinstance(other, DateTime):
            return self.minusDateTime(other)
        else:
    ...
    def floor(self, accuracy):
        ...
        if floored_ticks == self._ticks:
            return self
        return DateTime.makeTicks(floored_ticks, self._tz)
    ...
    def toTimeZone(self, tz):
        ...
        return DateTime.makeTicks(self._ticks, tz)

    def toUtc(self):
        """Convert to UTC timezone"""
        from .TimeZone import TimeZone
        return self.toTimeZone(TimeZone.utc())

    def toRel(self):
        """Convert to Rel timezone (relative timezone)"""
        from .TimeZone import TimeZone
        return self.toTimeZone(TimeZone.rel())

    def dayOfYear(self):
        """Get day of year (1-366)"""
        return self.date().dayOfYear()

    def weekOfYear(self, startOfWeek=None):
        """Get week of year"""
        return self.date().weekOfYear(startOfWeek)
    ...
    @staticmethod
    def weekdayInMonth(year, month, weekday, pos):
            ...
            if c == 'Y':  # Year
                ...
                if count == 1:
                    ...
                    if rule is not None:
                        ...
                        offset_secs = offset_dur.toSec() if hasattr(offset_dur, 'toSec') else offset_dur
                        if self.dst():
                            dst_off = self._tz.dstOffset(self._year)
                            if dst_off:
                                dst_secs = dst_off.toSec() if hasattr(dst_off, 'toSec') else dst_off
    ...
    @staticmethod
    def fromLocale(s, pattern, tz=None, checked=True):
        ...
        try:
            ...
            while i < pattern_len:
                ...
                if c == 'Y':
                    year = DateTime._parseInt(s, pos, count)
                    ...
                    if count >= 3:
                        mon, pos = DateTime._parseMonth(s, pos, month_abbr, month_full)
                    else:
                        mon = DateTime._parseInt(s, pos, count)
                        pos += len(str(mon)) if count == 1 else count
                elif c == 'D':
                    if count == 3:
                        # Day with suffix (1st, 2nd, etc.)
                        day = DateTime._parseInt(s, pos, 1)
                        pos += len(str(day))
                        # Skip suffix
                        while pos < len(s) and s[pos].isalpha():
                            pos += 1
                    else:
                        day = DateTime._parseInt(s, pos, count)
                    ...
                    hour = DateTime._parseInt(s, pos, count)
                    pos += len(str(hour)) if count == 1 else count
                elif c == 'm':
                    min_ = DateTime._parseInt(s, pos, count)
                    pos += len(str(min_)) if count == 1 else count
                elif c == 's':
                    sec = DateTime._parseInt(s, pos, count)
                    ...
                    if count == 1:
                        ...
                        tz_offset, pos = DateTime._parseTzOffset(s, pos)
                    else:
                        # Parse timezone name
                        tz_name, pos = DateTime._parseTzName(s, pos)
                elif c == "'":
                    # Quoted literal
                    if count == 2:
                        # Escaped quote
            ...
            if tz_name is not None:
                def_rule_std = tz.stdAbbr(year) if hasattr(tz, 'stdAbbr') else None
                def_rule_dst = tz.dstAbbr(year) if hasattr(tz, 'dstAbbr') else None
                if tz_name == tz.name() or tz_name == def_rule_std or tz_name == def_rule_dst:
                    result_tz = tz
                else:
                    try:
                        result_tz = TimeZone.fromStr(tz_name)
                    except:
                        result_tz = tz
                ...
                def_offset_secs = def_offset.toSec() if hasattr(def_offset, 'toSec') else def_offset
                ...
                if tz_offset == actual_offset:
                    ...
                    result_tz = TimeZone._fromGmtOffset(tz_offset)

            # If we have a known offset, use it for exact ticks calculation
            # This is crucial for half-hour offsets like -03:30
            if tz_offset is not None:
                return DateTime._makeWithOffset(year, mon, day, hour, min_, sec, ns, tz_offset, result_tz)
    ...
    @staticmethod
    def _parseMonth(s, pos, abbr_list, full_list):
    ...
    def toCode(self):
        ...
        if self.equals(DateTime.defVal()):
            return "DateTime.defVal"
        return f'DateTime("{self.toStr()}")'
    ...
    def fromIso(s, checked=True):
        ...
        try:
            ...
            tz = TimeZone._fromGmtOffset(offset_secs)
    ...
    @staticmethod
    def fromJava(millis, tz=None, negIsNull=True):
        ...
        return DateTime.makeTicks(fantom_ticks, tz)
    ...
    @staticmethod
    def fromHttpStr(s, checked=True):
...
class Date(Obj):
    ...
    def __init__(self, year_or_str, month=None, day=None):
        ...
        if isinstance(year_or_str, str):
            parsed = Date.fromStr(year_or_str)
            self._year = parsed._year
            self._month = parsed._month
            self._day = parsed._day
        else:
    ...
    @staticmethod
    def _numDaysInMonth(year, month_ordinal):
        """Get number of days in a month (0-based month ordinal)"""
        days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
        if month_ordinal == 1 and Date._isLeapYear(year):
            return 29
    ...
    def _dayOfYear(self):
        ...
        if Date._isLeapYear(self._year):
    ...
    def plus(self, duration):
        ...
        while num_days != 0:
            if num_days > 0:
                day += 1
                if day > Date._numDaysInMonth(year, month):
                    day = 1
                    month += 1
                    if month >= 12:
                        month = 0
                ...
                if day <= 0:
                    ...
                    if month < 0:
                        month = 11
                        year -= 1
                    day = Date._numDaysInMonth(year, month)
                num_days += 1
    ...
    def minusDate(self, that):
        ...
        if a._year == b._year:
            days = b._dayOfYear() - a._dayOfYear()
        else:
            days = (366 if Date._isLeapYear(a._year) else 365) - a._dayOfYear()
            days += b._dayOfYear()
            for i in range(a._year + 1, b._year):
                days += 366 if Date._isLeapYear(i) else 365
    ...
    def lastOfMonth(self):
        ...
        last_day = self._month.numDays(self._year)
    ...
    def lastOfQuarter(self):
        """Get last day of this quarter"""
        q = self.quarter()
        last_month = Month._get(q * 3 - 1)
        result = Date(self._year, last_month, last_month.numDays(self._year))
        if self.equals(result):
            return self
        return result
    ...
    def toDateTime(self, time, tz=None):
        """Convert to DateTime with given time"""
        from .TimeZone import TimeZone
        if tz is None:
            tz = TimeZone.cur()
        return DateTime(self._year, self._month, self._day,
                       time.hour(), time.min(), time.sec(), time.nanoSec(), tz)
    ...
    def weekOfYear(self, startOfWeek=None):
        ...
        if startOfWeek is None:
            startOfWeek = Weekday.localeStartOfWeek()
    ...
    def toCode(self):
        """Return Fantom code representation"""
        if self.equals(Date.defVal()):
            return "Date.defVal"
        return f'Date("{self.toStr()}")'
    ...
    @staticmethod
    def fromLocale(s, pattern, checked=True):
...
class Time(Obj):
    ...
    def now(tz=None):
        ...
        if now.tz() != tz:
            now = now.toTimeZone(tz)
        return Time(now.hour(), now.min(), now.sec(), now.nanoSec())
    ...
    def _plusTicks(self, ticks):
        ...
        new_ticks = self.toDuration().ticks() + ticks
        if new_ticks < 0:
        ...
        from .Duration import Duration
        return Time.fromDuration(Duration.make(new_ticks))

    def plus(self, duration):
        """Add duration (0-24hr range, with rollover)"""
        return self._plusTicks(duration.ticks())
    ...
    def toCode(self):
        """Return Fantom code representation"""
        if self.equals(Time.defVal()):
            return "Time.defVal"
        return f'Time("{self.toStr()}")'
    ...
    def toDateTime(self, date, tz=None):
